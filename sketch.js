...
function setup() {
  ...
  // faire apparaitre 10 particules de nourriture
  for (let i = 0; i < 10; i++) {
    spawnFood();
  }
}
...
function draw() {
  ...
  for (let shark of sharks) {
    ...
    if (closest) {
      ...
      if (d < 5) {
        ...
        // enlever le poisson du tableau flock ou eels (anguilles)
  ...
  for (let o of obstacles) {
   ...
   // montrer toutes les particules de nourriture
   for (let food of foodParticles) {
    food.show();
  }

  // première anguille mange la nourriture
  if (eels.length > 0) {
    let firstEel = eels[0];
    for (let i = foodParticles.length - 1; i >= 0; i--) {
      let food = foodParticles[i];
      if (firstEel.pos.dist(food.pos) < firstEel.r) {
        // anguille mange la nourriture
...
function growLastEel() {
  ...
  if (lastEel.r < 50) {
    ...
    // ajouter une nouvelle anguille si la derniere anguille a atteint la taille 50
...
function keyPressed() {
  if (key === 'd') {
    ...
    // creer un requin à une position aléatoire
    ...
    x = random(width);
    y = random(height);
    // creer 10 poissons avec des couleurs aléatoires
    for (let i = 0; i < 10; i++) {
      let randomFishImage = fishImages[Math.floor(random(fishImages.length))];
