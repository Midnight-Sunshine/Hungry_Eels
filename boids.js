...
class Boid {
  ...
  avoid(obstacles) {
    ...
    // calculer les points devant le vaisseau
    ...
    // debug visuel pour la force d'évitement (ligne jaune)
    if (Boid.debug) { // Draw debug vectors 
      this.drawVector(this.pos, ahead, "yellow");
   }

    // ajuster les points par rapport à la position actuelle du vaisseau
    ahead.add(this.pos);
    ahead2.add(this.pos);
    ahead3.add(this.pos);

    // debug visuel pour les points devant le vaisseau
    if (Boid.debug) {
        fill("red");
    ...
    // trouver l'obstacle le plus proche en utilisant la fonction getObstacleLePlusProche
    let closestObstacle = this.getObstacleLePlusProche(obstacles);

    if (closestObstacle) {
        // Calculer les distances entre les points devant le vaisseau et l'obstacle le plus proche
        ...
        // si il y a une collision potentielle, calculer la force d'évitement
        if (closestDistance < closestObstacle.r + this.r / 2) {
            let desiredSpeed;
            // determiner quel point a déclenché la collision
            ...
            // Calculer et appliquer la force d'évitement
